...
template <auto maker> auto create_options(const nil::clix::Options &options) {
  return maker(
      {.host = "127.0.0.1", .port = std::uint16_t(number(options, "port"))});
}

template <auto maker> auto create_ws_sc(const nil::clix::Options &options) {
  return maker({.host = "127.0.0.1",
...
auto input_output(auto &service) {
  ...
  while (std::getline(std::cin, message)) {
    ...
    publish(service, nil::service::concat(type, "typed > ", message, " : ",
                                          "secondary here"));
...
template <typename T, typename U> void loop(T &service, U &io) {
  while (true) {
...
template <typename T> void handlers(T &service) {
  on_ready(                                                     //
      service,                                                  //
      [](const auto &id) {                                      //
  ...
  on_disconnect(service,                                                  //
                [](const nil::service::ID &id) {                          //
                  std::cout << "disconnected : " << id.text << std::endl; //
                });
}

template <auto creator> int runner(const nil::clix::Options &options) {
  ...
  on_message(service,
             nil::service::map(
                 ...
                 nil::service::mapping(1u, [](const auto &id, const void *data,
                                              std::uint64_t size) {
                   const auto m =
                       nil::service::consume<std::string>(data, size);
                   std::cout << "from         : " << id.text << std::endl;
...
void sc_node(nil::clix::Node &node) {
  ...
  sub(node, "server", "server", [](auto &nn) {
    (option_adder(nn), ...);
    use(nn, runner<server_maker>);
  });
  sub(node, "client", "client", [](auto &nn) {
    (option_adder(nn), ...);
...
nil::service::P add_web_service(nil::service::WebService &server) {
  on_get(server, [](const nil::service::WebTransaction &transaction) -> void {
  ...
  on_message(ws,                                            //
             ...
             {
               std::cout << "message    : " << id.text << "  :  " << content
                         << std::endl;
             } //
  );
  handlers(ws);
...
void add_web_node(nil::clix::Node &node) {
  ...
  use(node, [](const nil::clix::Options &options) {
    if (flag(options, "help")) {
      help(options, std::cout);
      return 0;
    }
...
int main(int argc, const char **argv) {
  ...
  add_help(node);
  use(node, nil::clix::prebuilt::Help(&std::cout));
  sub(node, "self", "use self protocol", [](auto &n) {
    add_help(n);
    use(n, runner<&create_self>);
  });
  sub(node, "udp", "use udp protocol",
      sc_node<create_options<&nil::service::udp::server::create>,
              create_options<&nil::service::udp::client::create>, add_help,
              add_port>);
  sub(node, "tcp", "use tcp protocol",
      sc_node<create_options<&nil::service::tcp::server::create>,
              create_options<&nil::service::tcp::client::create>, add_help,
              add_port>);
  sub(node, "ws", "use ws protocol",
      sc_node<create_ws_sc<&nil::service::ws::server::create>,
              create_ws_sc<&nil::service::ws::client::create>, add_help,
              add_port, add_route>);
#ifdef NIL_SERVICE_SSL
  sub(node, "wss", "use wss protocol",
      sc_node< //
          create_wss_server, create_ws_sc<&nil::service::wss::client::create>,
          add_help, add_port, add_route>);
#endif
  sub(node, "http", "serve http server",
      &add_web_node< //
          create_http_server<&nil::service::http::server::create>, add_help,
          add_port>);

#ifdef NIL_SERVICE_SSL
  sub(node, "https", "serve https server",
      &add_web_node< //
          create_https_server<&nil::service::https::server::create>, add_help,
          add_port>);
#endif
